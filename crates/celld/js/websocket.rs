...
pub struct WorkerWebSocket {
    id: u64,
    /// `None` once `repark` returned the channel to the handoff registry: the
    /// caller isolate owns the socket then, and dropping this value must not
    /// unregister a channel it no longer holds.
