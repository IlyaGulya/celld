...
/// Every live slot, keyed by heap id, so an op running inside an isolate can
/// find the slot it is running in. That is what mints a process-local RPC
/// bridge handle: the handle names the origin isolate, and only a slot can be
/// turned into a drive for it.
...
/// The slot whose isolate is currently entered on this thread, if any.
